...
class Osu(Scene):
	...
	def setup(self):
		...
		for i,j in enumerate(self.bm):
			...
			if j[:11]=='CircleSize:':
				self.cs=cs(min(10,float(j[11:])*(1.4 if self.mod_hr else (.5 if self.mod_ez else 1)))-1)
			if j[:18]=='OverallDifficulty:':
				self.od=tuple(k/(1.5 if self.mod_dt else .75 if self.mod_ht else 1) for k in od(min(10,float(j[18:])*(1.4 if self.mod_hr else (.5 if self.mod_ez else 1)))-2))
		...
		while self.i<self.ct and ts[self.i]<=t+ar:
			...
			if ots[i][0]==8:
				...
				def temp():
					self.combo+=1
					self.i2+=1
				Timer(ar+(float(self.objs[i][5])/1000.-ts[i])*(1.5 if self.mod_dt else .75 if self.mod_ht else 1),temp).start()
				self.a.add_child(b)
			else:
