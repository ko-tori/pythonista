...
class Osu(Scene):
	...
	def setup(self):
		...
		for i,j in enumerate(self.bm):
			...
			if j[:11]=='CircleSize:':
				self.cs=cs(min(10,float(j[11:])*(1.4 if self.hr else (.5 if self.ez else 1))))
			if j[:18]=='OverallDifficulty:':
				self.od=od(min(10,float(j[18:])*(1.4 if self.hr else (.5 if self.ez else 1))))
		...
		self.combo=0
		self.combotemp=0
		self.cur=SpriteNode('Skin/cursor.png')
	...
	def update(self):
		...
		self.cur.position=x[self.i2],y[self.i2]
				...
				if ots[i][0]==2:
					...
					for rn in range(r-1):
						if rn==0:
							...
							c.rotation=angle
							dt=ar+(rn+1)*sdur/r
							c.run_action(A.sequence(A.wait(ar+sdur/r*(rn-1)),A.call(f_ra,sdur/r*2)))
							self.a.add_child(c)
...
#s=Osu("yuikonnu - Otsukimi Recital (Mythol) [iyasine's Hard].osu",'107763 Otsukimi Recital',mods)
